...
public class InvoiceController {
    ...
    public String deleteInvoiceLine(@PathVariable("invoiceId") int invoiceId,
                                    @PathVariable("invoiceLineId") int invoiceLineId,
    ...
    public String invoiceLine(@PathVariable(value = "invoiceId", required = false)
                                          String invoiceIdParameter, Model model) {
        ...
        BigDecimal sumValues = invoiceLines.stream()
                ...
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        Function<InvoiceLine, BigDecimal> totalMapperVat = line -> line.getQuantity().multiply(line.getPrice())
                .multiply(line.getVatRate()).divide(new BigDecimal("100"));
        BigDecimal sumVat = invoiceLines.stream()
                .map(totalMapperVat)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
        Function<InvoiceLine, BigDecimal> totalMapperTotal = line -> (line.getQuantity().multiply(line.getPrice()))
                .add(line.getQuantity().multiply(line.getPrice()).multiply(line.getVatRate())
